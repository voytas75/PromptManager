...
Status: offline stages 1–5 delivered; separately gated live probes not implemented
...
**Current recommended next decision:** keep the provider-free offline `doctor` delivered at `4e8c28fbb262ca00ec519c5ca5f8bef9de629a55` (exact-SHA Quality Gates success), with legacy aliases intact. If a concrete operator need arises for reachability diagnostics, separately authorize and budget a bounded `--live` embedding probe; optional analytics live probe and alias retirement remain independent decisions. No live provider call was made. All non-live routes listed above are callable; `--live` remains a contract proposal, **not yet callable**.
