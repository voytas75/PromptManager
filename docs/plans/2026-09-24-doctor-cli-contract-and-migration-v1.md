...
Status: offline stages 1–5 and `doctor embeddings --live` delivered; `doctor analytics --live` provider-free verified locally, live acceptance/delivery pending
...
**Current next decision:** opt-in `doctor embeddings --live` was authorized and exercised once using a synthetic text and the configured Azure/LiteLLM embedding backend. The installed CLI returned exit 0, `EMBEDDING_BACKEND_REACHABLE`, dimension 3072, and no stderr. That proves this request returned a usable vector, **not** vector-index health or retrieval quality. No second live call is needed for this acceptance. Provider-free local gates passed (957 tests passed, 1 skipped; coverage 80.97%, Ruff, strict Pyright, lock and wheel); exact-SHA Quality Gates [36008958726](https://github.com/voytas75/PromptManager/actions/runs/36008958726) passed for delivered `355e6503b5ca1f618733dea9f77d063eb01cddc3`; the provider-backed acceptance was one synthetic request, not a CI step. Default/offline routes stay provider-free; `doctor analytics --live` has provider-free local verification (965 passed, 1 skipped; core coverage 81.41%; Ruff, strict Pyright, lock and wheel); fake-provider entrypoint routing covers both module paths. No second real provider probe was run. The next decision is whether to authorize one bounded live acceptance request against the configured backend and separately whether to publish this local checkpoint. Legacy alias retirement and vector-index inspection remain separate decisions.
