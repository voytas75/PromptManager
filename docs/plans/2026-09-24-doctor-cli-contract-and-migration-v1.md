...
Status: stage 1 delivered; stages 2–4 and stage 5 local docs/verification complete; remote delivery and separately gated live probes pending
...
- `doctor embeddings` without `--live` checks effective configuration only and marks backend/vector reachability unprobed; it does not generate embeddings or inspect the vector index. `--live` is an unimplemented contract proposal for an explicit actual backend probe; it may call a paid/remote provider and must say so in help before execution. Preserve the existing `diagnostics embeddings` legacy behavior separately. A future `--fix` is out of scope and requires its own risk decision.
...
**Current recommended next execution slice:** delivery review of the local stages 2–5 change set (scope, staged diff, commit/push authorization, exact-SHA CI), then separately decide whether `--live` embedding/analytics probes are needed. Live provider acceptance requires explicit budget/approval; no such call was made. Stage 1 was pushed as `7487bbf` to `origin/master` and its exact-SHA Quality Gates succeeded. Stages 2–5 are locally verified, not yet remote-delivered. All non-live routes listed above are callable; `--live` remains a contract proposal, **not yet callable**.
