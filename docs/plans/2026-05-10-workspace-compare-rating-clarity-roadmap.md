# PromptManager Workspace Compare-Rating Clarity Roadmap

> **For Hermes:** Use subagent-driven-development skill to implement this plan task-by-task.

**Goal:** Reduce hesitation on compare decisions by making one bounded missing-rating evidence path more explicit on the existing workspace history decision-support seam, without reopening delivered missing-baseline wording, prompt-detail handoff work, or broader history UX.

**Architecture:** This roadmap stays inside the existing trust-surface seam in `gui/workspace_history_controller.py`, specifically where recent execution evidence is converted into compact next-action guidance for comparison readiness. The slice must remain bounded: audit the current missing-rating contract, reproduce the wording gap with one focused test, land one compact next-action clarification, and avoid any change to run-summary structure, provenance logic, or unrelated evidence branches.
...
Most recently delivered bounded ledger before this slice: `docs/plans/2026-05-10-workspace-compare-readiness-clarity-roadmap.md`
...
- `tests/test_workspace_history_controller.py` already covers the missing-rating path, so the next safe move is to extend the existing controller test surface rather than introduce a new product seam.

So the next useful move is:

**workspace trust surface -> make the missing-rating compare path more explicit without broadening history UX**
...
- changes to missing-baseline or missing-duration wording unless required by a focused regression.

---

## Main product question for this roadmap

> what is the smallest compare-readiness clarification that makes the missing-rating path more actionable without expanding the workspace-history model?
...
## Stage A — Missing-rating compare clarity

### Task 1: Audit the current missing-rating contract

**Status:** completed

**Objective:** Confirm the exact current evidence and next-action strings for the missing-rating path and identify the smallest wording gap.

**Files:**
- Inspect: `gui/workspace_history_controller.py`
- Inspect: `tests/test_workspace_history_controller.py`
- Maintain: `docs/plans/2026-05-10-workspace-compare-rating-clarity-roadmap.md`

**Audit checks:**
- confirm where `Evidence: missing rating for comparison` is emitted,
- confirm what next action currently maps from that evidence,
- confirm whether the current wording is ambiguous about which runs need ratings before comparison.
...
**Objective:** Add one failing test that proves the current missing-rating compare guidance is less explicit than desired.
...
**RED target shape:**
- construct the existing missing-rating history scenario,
- assert current decision support reaches the bounded evidence path,
- expect one more explicit rating-oriented operator action string.

**Exit criterion:**
One focused failing controller test for the missing-rating wording/mapping gap.

---

### Task 3: GREEN minimal compare-rating clarification

**Status:** completed

**Objective:** Land the smallest wording or mapping change that makes the missing-rating path more actionable.
...
- keep behavior bounded to the missing-rating path,
- do not change missing-baseline, missing-duration, or freshness branches.
...
**Exit criterion:**
Missing-rating compare-readiness seam is green and docs identify the most recently delivered bounded slice correctly.
...
- one missing-rating compare path becomes more explicit,
- missing-baseline and missing-duration contracts remain intact,
