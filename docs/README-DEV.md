...
| `python -m main note [--limit N] [--json]` | List the last 20 standalone GUI Notes from the existing `prompt_notes` SQLite table, newest first; limit 1–100. Both CLI entrypoints bypass manager/Chroma/provider bootstrap. Reads require an existing compatible catalog and never create it. Lists show IDs, first-line previews and modification time in JSON. |
| `python -m main note add [TEXT\|--body TEXT\|--file PATH\|--from-stdin] [--json]` | Add a standalone note to the existing catalog (not a prompt). One nonblank UTF-8 source, up to 1 MiB; text is stripped as in GUI. JSON returns the created full record and ID. No catalog bootstrap. |
| `python -m main note show <uuid> [--json]` | Read full note text, UUID, and timestamps. Full canonical UUID only; no prefix search. |
| `python -m main note find <literal> [--limit N] [--json]` | Find literal text in standalone notes with bounded newest-first output. SQL `%`, `_`, and escape characters are treated literally; SQLite LIKE is case-insensitive for ASCII, not full Unicode casefolding. |
| `python -m main note edit <uuid> [TEXT\|--body TEXT\|--file PATH\|--from-stdin] [--json]` | Replace a note's text while retaining identity/creation time; require exactly one nonblank source. |
| `python -m main note delete <uuid> [--yes] [--json]` | Permanently delete a note from the same GUI table. Asks on interactive stdin with default No; without a TTY or in JSON mode requires `--yes`. No soft-delete. JSON success emits one object; errors go only to stderr with a nonzero exit. |
| `python -m main draft [--limit N] [--json]` | List recent draft **prompts** in the existing `prompts` SQLite table; unlike standalone Notes, drafts have `ext2.capture_state=draft`. Limit 1–100, default 20; newest modified first. Reads do not bootstrap a catalog, Chroma, or providers. |
| `python -m main draft add [TEXT\|--body TEXT\|--file PATH\|--from-stdin] [--title TITLE] [--source SOURCE] [--json]` | Create a new CLI-origin draft UUID from one nonblank UTF-8 body (at most 1 MiB). An omitted title is derived locally from body; preserves the given body (no GUI wrapper cleanup). Records a version and CLI activity. Never calls a provider or adds an embedding/index entry; the draft is not semantically searchable until a separate indexing workflow. |
| `python -m main draft show <uuid> [--json]` | Show the current draft body and compact metadata, without `ext4`. Requires a full canonical UUID still marked draft. |
| `python -m main draft find <literal> [--limit N] [--json]` | Local literal SQLite search over title, description and body of drafts, newest first. `%`, `_` and escape characters are literal; SQLite LIKE is ASCII-case-insensitive, not semantic search. |
| `python -m main draft delete <uuid> [--yes] [--json]` | Confirmed permanent deletion of a still-draft prompt; non-TTY/JSON requires `--yes`, interactive confirmation defaults No. Requires Redis caching to be **unconfigured** (`CACHE_CONFIGURED` otherwise), to avoid leaving a deleted prompt body in the manager cache. Under a catalog write lock, rechecks draft status, inspects existing Chroma metadata read-only and cleans a matching vector only if present, then removes the prompt/dependent records and records CLI deletion activity. Index inspection failure retains the SQLite draft; vector cleanup or a later catalog failure can report `PARTIAL_DELETE` (inspect both stores). Promotion is a separate future CLI slice. |
...
| `python -m main prompt-edit <uuid> set --attr related_prompts --value '[]' [--expect-value '["[]"]' --apply --backup-to PATH] [--json]` | Provider-free, UUID-only, single-field edit: default is a read-only preview. `--value` is a JSON array of unique canonical UUIDs present in the selected catalog (or empty `[]`); v1 allowlists only `related_prompts`. Apply requires the current JSON value and a new non-existing SQLite backup path. The edit compares under a write lock, creates an integrity-checked SQLite backup before updating only `related_prompts`, then reads back the committed field. No vector/index changes, prompt body rewrite, activity event or version snapshot; keep GUI/other writers closed during maintenance. Text output is readable; `--json` emits a single result on stdout, bounded errors on stderr; exit 0 success/preview/no-op, 2 rejected edit. |
