...
The installed wheel exposes `prompt-manager`; the `python -m main` forms below remain equivalent repository-checkout commands. Bare `doctor` provides shallow readiness; `doctor catalog` audits records; `doctor config` and `doctor embeddings` are focused offline views. `doctor analytics` is a local counts report, not a health claim. Targeted prompt/chain routes use provider-free validators. Live probes remain planned; standalone commands stay supported.
...
| `python -m main doctor embeddings [--json]` | Offline embedding configuration readiness only; no provider call, vector index mutation, or claim of successful connectivity. Legacy `diagnostics embeddings` retains its live behavior. |
