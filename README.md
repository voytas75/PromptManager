...
- Response Style registry stores tone/formatting presets with inline CRUD controls so operators can define reusable answer guidelines directly from the GUI; paste any response phrase and the dialog auto-generates the remaining metadata.
- Notes tab (next to Prompts/History) keeps lightweight prompt notes with single-field CRUD, copy-to-clipboard, and export-to-file controls for quick reminders.
