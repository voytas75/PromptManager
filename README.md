...
- Prompt chaining pipelines with CLI management (`prompt-chain-*` commands) and a dedicated GUI dialog (toolbar button) so teams can create/edit/delete chains, validate inputs, and replay workflows with a single command or via the desktop app, capturing per-step history and outputs.
