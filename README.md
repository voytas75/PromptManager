...
export PROMPT_MANAGER_WEB_SEARCH_PROVIDER="tavily"  # or "exa" / "serper" / "serpapi" / "random"
...
- Provider-agnostic web search integrations (now supporting Exa, Tavily, Serper, and SerpApi) plus a workspace “Use web search” toggle (on by default) that injects every available snippet (summary + highlights) before each run—when those snippets exceed roughly 5,000 words, the fast LiteLLM model condenses them automatically so prompts still stay concise.
...
- Prompt sharing workflows baked into the prompt detail pane: choose whether to publish just the body, body + description, or body + description + scenarios, optionally include metadata, and then pick ShareText or a PrivateBin instance (AES-256-GCM per the [PrivateBin API](https://github.com/PrivateBin/PrivateBin/wiki/API), retrieved 2025-12-07). The app handles encryption/uploading and copies the share URL (plus the delete token for ShareText/PrivateBin) to your clipboard automatically.
...
The staged web search rollout plan (covering Exa, Tavily, Serper, and SerpApi) lives in [`docs/web_search_plan.md`](docs/web_search_plan.md).
