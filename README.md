...
- Paste code or free-form context into the workspace, select a prompt, and click **Run Prompt**. Review raw responses in the **Output** tab, keep iterating via **Continue Chat**, and rely on **Save Result** to capture the run in history with optional notes while **Copy Result** sends the generated text to the clipboard.
