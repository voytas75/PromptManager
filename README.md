...
- Prompt chaining pipelines with CLI management (`prompt-chain-*` commands) and a dedicated GUI dialog (toolbar button) so teams can compose sequential prompt workflows, validate inputs, and replay chains with a single command or via the desktop app, capturing per-step history and outputs.
