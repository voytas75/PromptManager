...
- Provider-agnostic web search integrations (starting with Exa) plus a workspace “Use web search” toggle (on by default) that injects live references before each run, so authors can gather fresh context without editing prompts.
