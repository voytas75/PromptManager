...
from gui.layout_state import (
    EXECUTE_CONTEXT_HISTORY_KEY,
    EXECUTE_CONTEXT_TASK_KEY,
    FILTER_CATEGORY_KEY,
    FILTER_QUALITY_KEY,
    FILTER_SORT_KEY,
    FILTER_TAG_KEY,
    load_execute_context_history,
    load_filter_preferences,
    load_last_execute_context_task,
    store_execute_context_history,
    store_filter_preferences,
    store_last_execute_context_task,
...
def test_load_last_execute_context_task_returns_trimmed_value() -> None:
    ...
    settings.values[EXECUTE_CONTEXT_TASK_KEY] = "  Summarise logs  "

    assert load_last_execute_context_task(_as_qsettings(settings)) == "Summarise logs"
...
def test_store_last_execute_context_task_persists_text_and_syncs() -> None:
    ...
    store_last_execute_context_task(_as_qsettings(settings), "Investigate timeouts")

    assert settings.values[EXECUTE_CONTEXT_TASK_KEY] == "Investigate timeouts"
...
def test_load_execute_context_history_preserves_whitespace() -> None:
    ...
    settings = _FakeSettings()
    settings.values[EXECUTE_CONTEXT_HISTORY_KEY] = json.dumps(
        ["  Summarise logs  ", "Summarise logs", "Investigate outages", " "]
    )

    entries = load_execute_context_history(_as_qsettings(settings), limit=3)
...
def test_load_execute_context_history_handles_invalid_payload() -> None:
    ...
    settings = _FakeSettings()
    settings.values[EXECUTE_CONTEXT_HISTORY_KEY] = "{"  # invalid JSON

    assert load_execute_context_history(_as_qsettings(settings)) == []
...
def test_store_execute_context_history_preserves_whitespace_and_limits() -> None:
    ...
    stored_raw = settings.values[EXECUTE_CONTEXT_HISTORY_KEY]
...
def test_load_filter_preferences_returns_trimmed_state() -> None:
    ...
    settings = _FakeSettings()
    settings.values[FILTER_CATEGORY_KEY] = "  incident_response  "
    settings.values[FILTER_TAG_KEY] = "  outages  "
    settings.values[FILTER_QUALITY_KEY] = "4.5"
    settings.values[FILTER_SORT_KEY] = "usage_desc"

    category, tag, quality, sort_value = load_filter_preferences(_as_qsettings(settings))
...
def test_load_filter_preferences_handles_invalid_quality() -> None:
    ...
    settings = _FakeSettings()
    settings.values[FILTER_QUALITY_KEY] = "fast"

    _, _, quality, sort_value = load_filter_preferences(_as_qsettings(settings))
...
def test_store_filter_preferences_persists_values_and_syncs() -> None:
    ...
    assert settings.values[FILTER_CATEGORY_KEY] == "incident_response"
    assert settings.values[FILTER_TAG_KEY] == "outages"
    assert settings.values[FILTER_QUALITY_KEY] == 3.2
    assert settings.synced is True
...
def test_store_sort_preference_persists_value() -> None:
    ...
    store_sort_preference(_as_qsettings(settings), PromptSortOrder.USAGE_DESC)

    assert settings.values[FILTER_SORT_KEY] == PromptSortOrder.USAGE_DESC.value
    assert settings.synced is True
